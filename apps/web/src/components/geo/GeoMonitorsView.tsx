...
function ScoreBar({ value, max = 100 }: { value: number; max?: number }) {
  ...
  const color = pct >= 70 ? 'bg-sx-success' : pct >= 40 ? 'bg-sx-warning' : 'bg-sx-error'
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-sx-surface-2 rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-700 ${color}`} style={{ width: `${pct}%` }} />
      </div>
      <span className={`text-xs font-bold w-8 text-right ${
        pct >= 70 ? 'text-sx-success' : pct >= 40 ? 'text-sx-warning' : 'text-sx-error'
      }`}>
        {Math.round(value)}
      </span>
    </div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2].map(i => (
            <div key={i} className="sx-card animate-pulse">
              <div className="h-4 bg-sx-surface-2 rounded w-1/3 mb-2" />
              <div className="h-3 bg-sx-surface-2 rounded w-2/3" />
            </div>
          ))}
        </div>
      ) : monitors.length === 0 ? (
        ...
        <div className="space-y-4">
          {monitors.map((m) => {
            ...
            return (
              <div key={m.id} className="sx-card space-y-4">
                {/* Monitor header */}
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="sx-btn-primary px-3 py-1.5 text-xs flex items-center gap-1.5 disabled:opacity-60"
                    >
                      {isCollecting ? (
                        <>
                    ...
                    <button
                      ...
                      className="text-sx-text-3 hover:text-sx-error transition-colors p-1"
                ...
                {score && (
                  <div className="border-t border-sx-border pt-4 space-y-3">
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-xs font-semibold text-sx-text-2 uppercase tracking-wider">GEO Score</p>
                      <p className="text-xs text-sx-text-3">
                        {new Date(score.collectedAt).toLocaleString('pt-BR', { timeStyle: 'short', dateStyle: 'short' })}
                      </p>
                    </div>

                    {/* Overall score */}
                    <div className="flex items-center gap-4 p-3 bg-sx-surface-2 rounded-sx-sm">
                      <div className="text-center w-16 shrink-0">
                        <p className={`text-3xl font-black ${
                          score.overallScore >= 70 ? 'text-sx-success' :
                          score.overallScore >= 40 ? 'text-sx-warning' : 'text-sx-error'
                        }`}>
                          {Math.round(score.overallScore)}
                        </p>
                        <p className="text-[10px] text-sx-text-3 mt-0.5">Score geral</p>
                      </div>
                      <div className="flex-1">
                        <p className="text-xs text-sx-text-3 mb-1">Visibility Rate</p>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-sx-border rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all duration-700 ${
                                parseFloat(score.mentionRate) >= 70 ? 'bg-sx-success' :
                                parseFloat(score.mentionRate) >= 40 ? 'bg-sx-warning' : 'bg-sx-error'
                              }`}
                              style={{ width: `${score.mentionRate}%` }}
                            />
                          </div>
                          <span className="text-xs font-semibold text-sx-text w-12 text-right">{score.mentionRate}%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {Object.entries(score.engineScores).map(([engine, val]) => (
                        <div key={engine} className="flex items-center gap-3">
                          <span className={`text-xs font-medium w-20 shrink-0 ${ENGINE_COLORS[engine] ?? 'text-sx-text-2'}`}>
