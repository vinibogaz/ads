...
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'orffia_app') THEN
    CREATE ROLE orffia_app LOGIN PASSWORD 'CHANGE_IN_PRODUCTION';
  END IF;
END $$;

GRANT CONNECT ON DATABASE orffia TO orffia_app;
GRANT USAGE ON SCHEMA public TO orffia_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO orffia_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO orffia_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO orffia_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO orffia_app;
`
